...
function bricks(rainbow) {
  return function (context, data, options) {
    ...
    var height = d3.scaleLinear().domain([0, 1]).range([0, waveHeight]);
    ...
    var barWidth = barX.bandwidth(),
      ...
      maxBricks = Math.max(
        1,
        Math.floor(waveHeight / (brickHeight + brickGap))
      );

    data.forEach(function (val, i) {
      var bricks = Math.max(
          1,
          Math.floor(height(val[0]) / (brickHeight + brickGap))
        ),
        x = barX(i);
      ...
      d3.range(bricks).forEach(function (b) {
        ...
        context.fillRect(
          x,
          options.waveBottom - brickHeight * (b + 1) - brickGap * b,
          barWidth,
          brickHeight
        );
      });
